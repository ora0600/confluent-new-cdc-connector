...
One additional hint(Needs to be tested): Please use GDS with your RAC instance, see [Oracle Documentation](https://docs.oracle.com/en/database/oracle/oracle-database/26/haovw/oracle-global-data-services-best-practices.html#GUID-D1E1C05D-0BE9-4E1C-B759-D3D0F69133FA).
GDS is designed to prevent that one node in your RAC cluster is overloaded. It provides automated workload routing and load balancing across the RAC nodes at the DR site. Without it, connections can concentrate on a single node, creating a bottleneck that would explain the drop in commit performance.
